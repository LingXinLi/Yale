...
class Citizen:
    ...
    def _str_indented(self, depth: int = 0) -> str:
        ...
        for subordinate in self.get_direct_subordinates():
            # Note that the 鈥榙epth鈥�argument to the recursive call is
            # modified.
    ...
    def become_subordinate_to(self, superior: Optional[Citizen]) -> None:
        ...
        []
        """
        old = self.get_superior()
        if old:
            old.remove_subordinate(self.cid)
        if superior:
    ...
    def get_citizen(self, cid: int) -> Optional[Citizen]:
        ...
        for subordinate in self._subordinates:
            if subordinate.get_citizen(cid):
    ...
    def get_all_subordinates(self) -> List[Citizen]:
        ...
        for sub in self._subordinates:
            result = merge(result, sub.get_all_subordinates())
        return result
        # Note: This method must call itself recursively
    ...
    def get_closest_common_superior(self, cid: int) -> Citizen:
        ...
        True
        >>> c5.get_closest_common_superior(5) == c5
        True
    ...
    def get_highest_rated_subordinate(self) -> Citizen:
        ...
        for s in self._subordinates:
            r = s.rating
            rating_lst.append(r)
        biggest = max(rating_lst)
        for sub in self._subordinates:
            if sub.rating == biggest:
                return sub
        return None
...
class Society:
    ...
    def get_all_citizens(self) -> List[Citizen]:
        ...
        if not self._head:
            ...
            return merge(head, self._head.get_all_subordinates())
    ...
    def add_citizen(self, citizen: Citizen, superior_id: int = None) -> None:
        ...
        if superior_id is None:
            if self._head is None:
                ...
                old = self._head
                self._head = citizen
                for s in citizen.get_all_subordinates():
                    citizen.remove_subordinate(s.cid)
                citizen.add_subordinate(old)
        elif superior_id is not None:
    ...
    def change_citizen_type(self, cid: int,
        ...
        if isinstance(obj, DistrictLeader):
            new = Citizen(cid, obj.manufacturer, obj.model_year,
                          obj.job, obj.rating)

        else:
            new = DistrictLeader(cid, obj.manufacturer, obj.model_year,
                                 obj.job, obj.rating, district_name)

        superior = obj.get_superior()
        if superior:
            superior.remove_subordinate(obj.cid)
            superior.add_subordinate(new)
        else:
            self._head = new

        for c in obj.get_direct_subordinates():
            new.add_subordinate(c)
    ...
    def _swap_up(self, citizen: Citizen) -> Citizen:
        ...
        citizen.cid, sup.cid = sup.cid, citizen.cid
        citizen.manufacturer, sup.manufacturer = \
            sup.manufacturer, citizen.manufacturer
        citizen.model_year, sup.model_year = sup.model_year, citizen.model_year
        citizen.rating, sup.rating = sup.rating, citizen.rating
        sup_sup = sup.get_superior()
        if sup_sup:
            sup_sup.remove_subordinate(sup.cid)
            sup_sup.add_subordinate(sup)
        sup.remove_subordinate(citizen.cid)
        sup.add_subordinate(citizen)
        return sup
    ...
    def promote_citizen(self, cid: int) -> None:
        ...
        """
        obj = self.get_citizen(cid)
        while not isinstance(obj, DistrictLeader) and obj.get_superior() \
                and obj.rating > obj.get_superior().rating:
            obj = self._swap_up(obj)
        return None
    ...
    def delete_citizen(self, cid: int) -> None:
        ...
        if self.get_head() != c:
            superior_of_c = c.get_superior()
            for sub in c.get_direct_subordinates():
                superior_of_c.add_subordinate(sub)
            superior_of_c.remove_subordinate(c.cid)
        elif self.get_head() == c:
            if not c.get_direct_subordinates():
                self._head = None
            else:
                highest = c.get_highest_rated_subordinate()
                self._head = highest
                c.remove_subordinate(highest.cid)
                for sub in c.get_direct_subordinates():
                    highest.add_subordinate(sub)
        return None
